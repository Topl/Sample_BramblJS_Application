...
class AddressesService {
    ...
    static async create(args) {
        ...
        try {
            ...
            const [balances, keyfile, address] = await AddressesService.getBalancesForAddress(
                args.address,
                args.network,
                args.password
            );
            ...
            if (fetchedUser.doc) {
                ...
                await save2db([fetchedUser.doc, newAddress], {
                    ...
                    if (result.error) {
                        ...
                        if (balances.boxes) {
                            return BoxHelper.updateBoxes(balances.boxes, address).then(function (result) {
                                if (result.error) {
                                    throw stdErr(500, "Unable to update application view", result.error, serviceName);
                                } else {
                                    return result;
                                }
                            });
                ...
                await save2db(newAddress, { timestamp, serviceName, session })
                    .then(function (result) {
                        if (result.error) {
                            ...
                            // iterate through boxes and either add or update them in the DB
                            if (balances.boxes) {
                                return BoxHelper.updateBoxes(balances.boxes, args.address).then(function (result) {
                                    if (result.error) {
                                        throw stdErr(
                                            500,
            ...
            if (err.name === "MongoError" && err.code === 11000) {
                throw stdErr(422, "The provided address is already in use", serviceName, serviceName);
            } else {
                throw err;
            }
        } finally {
    ...
    static async updateAddressByAddress(args) {
        ...
        let obj = {};
        const self = this;
        return await checkExists(Address, args.addressId, "address").then(function (result) {
            if (result.error) {
                throw stdErr(500, "Unable to update address by address", serviceName, serviceName);
            }
        ...
        const fetchedAddress = await checkExistsById(Address, args.addressId)
            .then(function (result) {
                if (!result.doc.isActive.status) {
                    throw stdErr(404, "No Active Address Found", serviceName, serviceName);
                }
                return result.doc;
            })
            ...
            .catch(function (err) {
                console.error(err);
                throw stdErr(500, "Unable to update address by ID", serviceName, serviceName);
            });
    ...
    static async updateAddress(args, fetchedAddress) {
        ...
        try {
            ...
            // save
            await save2db(fetchedAddress, { timestamp, serviceName, session }).then(function (result) {
                if (result.error) {
                    throw stdError(500, result.error, serviceName, serviceName);
                }
                return checkExists(Address, fetchedAddress.address, "address").then(function (result) {
                    if (result.error) {
                        throw stdError(500, result.error, serviceName, serviceName);
                    }
                    return result.doc;
    ...
    static async getAddressByAddress(args) {
        ...
        return await checkExists(Address, args.address, "address").then(function (result) {
            if (result.error) {
    ...
    static async getBalancesForAddress(address, network, password) {
        ...
        if (address) {
            ...
            brambl = new BramblHelper(bramblHelperParams);
            const generatedAddress = await brambl.createAddress().catch(function (err) {
                console.error(err);
        ...
        if (!balances) {
            throw stdErr(500, "Unable to retrieve balance for address from network", serviceName, serviceName);
        } else if (balances.polyBalance < 0) {
            throw stdErr(500, "polyBalance for address must be greater than 0.", serviceName, serviceName);
        }
