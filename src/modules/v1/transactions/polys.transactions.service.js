...
class PolyTransactionService {
    ...
    static async generateRawPolyTransfer(args) {
        return PolyTransfer.createRaw(args.recipients, args.sender, args.changeAddress, args.fee, args.data).then(
            function (value) {
                if (value.error) {
                    return value;
                } else {
    ...
    static async polyTransactionHelper(bramblHelper, args) {
        return bramblHelper.sendRawPolyTransaction(args).then(function (rpcResponse) {
            if (rpcResponse.error) {
                ...
                return PolyTransactionService.generateRawPolyTransfer(args).then(function (jsResponse) {
                    if (jsResponse.error) {
                        return jsResponse;
                    }
    ...
    static async polyTransaction(args) {
        ...
        const bramblHelperParams = {
            ...
            keyFile: args.keyfiles.length > 0 ? args.keyfiles[0] : null,
        };
        ...
        if (bramblHelper) {
            ...
            return PolyTransactionService.polyTransactionHelper(bramblHelper, bramblParams).then(function (result) {
                if (result.error) {
            ...
            throw stdError(404, "Missing or Invalid Private Key", serviceName, serviceName);
        }
    }
}
