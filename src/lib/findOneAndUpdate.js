...
const runTransactionWithRetry = async (_models, _updates, _filters, _serviceName, _upsert, _session) => {
    try {
        const docs = []
        // find and update the document using the session
        if (_models.length !== 0 && _updates.length !== 0 && _models.length == _updates.length) {
            for (var i = 0; i < _models.length; i++ ) {
                const doc = await Promise(((resolve, reject) => {
                    _models[i].findOneAndUpdate(
                        _filters[i], 
                        _updates[i],
                        {upsert: _upsert, new: true, session: _session}
                    );
                }))
                docs.push(doc)
            }
        ...
        if (error.errorLabels && error.errorLabels.indexOf('TransientTransactionError') >= 0) {
            ...
            const doc = await runTransactionWithRetry(_model, _update, _id, _args, _serviceName, _upsert, _session)
            return doc 
        } else {
...
module.exports = async(models, updates, filters, opts = {}) => {
    ...
    try {
        ...
        const doc = await runTransactionWithRetry(models, updates, filters, serviceName, upsert, session)
        return doc
    } catch (error) {
        throw error
    } finally {
