...
const runTransactionWithRetry = async (_models, _filters, _serviceName, _session) => {
    try {
        const docs = []

        if (_models.length !== 0 && _filters.length !== 0 && _models.length === _filters.length) {
        ...
        if (error.errorLabels && error.errorLabels.indexOf('TransientTransactionError') >= 0) {
            ...
            const docs = await runTransactionWithRetry(_models,_filters,_serviceName, _session, _adminOnly)
            return docs
        } else {
            // this will rollback any changes made in the DB
            await _session.abortTransaction
...
module.exports = async(models, filters, opts = {}) => {
    ...
    try {
        ...
        const doc = await runTransactionWithRetry(models,filters, serviceName,session, adminOnly)
        return doc
    } catch (error) {
        throw error
    } finally {
