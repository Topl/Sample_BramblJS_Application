...
class BramblHelper {
    ...
    generateBoxes(obj, result, address) {
        ...
        obj.boxes = result.result[address].Boxes.PolyBox
            ? result.result[address].Boxes.PolyBox.map((box) => BoxUtils.convertToBox(box))
            : [];
        obj.boxes = result.result[address].Boxes.ArbitBox
            ? obj.boxes.concat(result.result[address].Boxes.ArbitBox.map((box) => BoxUtils.convertToBox(box)))
            : obj.boxes;
        obj.boxes = result.result[address].Boxes.AssetBox
            ? obj.boxes.concat(result.result[address].Boxes.AssetBox.map((box) => BoxUtils.convertToBox(box)))
            : obj.boxes;
        return obj;
    }
        ...
        return new Promise((resolve, reject) => {
            let e = self.brambljs.requests
                ...
                .then(function (result) {
                    ...
                    obj.result.newBoxes = obj.result.newBoxes.map((box) => BoxUtils.convertToBox(box));
                    resolve(obj);
                })
                .catch(function (err) {
                    console.error(err);
